...
**v1.1+v1.2 Complete** — All v1.1 milestones and v1.2 structural/security upgrades implemented. Branch `feature/v1.1-admin-dashboard` merged and deleted.
...
| 27 | 2026-06-23 | Structural upgrade: OSS files (LICENSE, CONTRIBUTING, CODE_OF_CONDUCT, SECURITY, CHANGELOG), OpenAPI spec (docs/api/openapi.yaml), .prettierrc, .dockerignore. Tests: pkg/qr/qr_test.go (12 tests, fixed missing import), pkg/observability/observability_test.go (13 tests). APIError typed struct with consistent JSON error responses across all handlers. RequestID middleware for log correlation (X-Request-ID header). CORS middleware controlled by CORS_ORIGIN env var. Retry-After header on 429 responses. CI/CD pipeline (.github/workflows/ci.yml with 3 jobs: backend test + vet + build + vulncheck, frontend lint + type-check + build, Docker build smoke test). go mod tidy fixed `// indirect` markings on direct deps. Makefile dev-hot target for air hot reload. docker-compose.yml healthcheck configuration. Deleted stale feature/v1.1-admin-dashboard branch (local + remote). All 120+ tests pass with race detection, 50/50 security checks pass. |
| 28 | 2026-06-23 | Security hardening + observability: SecurityHeaders middleware (CSP, X-Frame-Options, X-Content-Type-Options, Referrer-Policy, Permissions-Policy), RequestLogger middleware (method/path/status/duration/reqID log), GzipMiddleware (automatic gzip compression), HTTP server timeouts (Read 10s, Write 15s, Idle 60s), enhanced health endpoint (version `1.2.0`, uptime, started_at RFC3339, goroutines, memory_alloc, memory_sys, gc_cycles), camera fallback in reader.html (rear→front camera auto-fallback). serverVersion constant. Fix frontend CI (eslint-plugin-react-refresh dep, lint and TS errors). Docs updated: README.md, docs/OVERVIEW.md, .claude/state/CURRENT_STATUS.md. New file: docs/plans/zero-drop-v1.2-upgrade-recommendations.md. All 120+ tests pass, 50/50 security checks pass, go vet clean. |
| 3 | 2026-05-11 | PRD revision v1.1: Frontend stack changed to React + Vite + shadcn/ui |
| 4 | 2026-05-11 | M-01 implementation: Go module, pkg/crypto, pkg/config |
| 5 | 2026-05-11 | M-02 implementation: pkg/api, pkg/spooler, pkg/observability |
| 6 | 2026-05-11 | M-03 implementation: pkg/printer, static/reader.html |
